...
Public Class GestioneFilesDirectory
	...
	Public Function AggiungeCampoACSV(Valore As String, NomeCampo As String, Origine As String, Destinazione As String, Dove As Integer, Separatore As String, Optional Header As Boolean = True) As String
		' DOVE -> Posizione dove inserire il valore. Va da 1 al numero massimo di campi
		...
		Dim RitornoFunzione As String = ""

		outputFile = New StreamWriter(Destinazione, True)

		If File.Exists(Origine) Then
			Try
				Dim objReader As StreamReader = New StreamReader(Origine)
				Dim sLine As String = ""
				...
				Do
					...
					If sLine <> "" Then
						If Not sLine.Contains(Separatore) Then
							Ritorno = "ERROR: Separatore non rilevato"
							Exit Do
						End If
				...
				If Ritorno.Contains("ERROR:") Then
					RitornoFunzione = Ritorno
				Else
					If Not Aggiunto Then
						RitornoFunzione = "ERROR: Nessuna riga modificata"
					Else
						RitornoFunzione = "OK"
					End If
				End If
			Catch ex As Exception
				RitornoFunzione = "ERROR: " & ex.Message
			End Try
		Else
			RitornoFunzione = "ERROR: File di origine non esistente"
		End If

		outputFile.Flush()
		outputFile.Close()

		Return RitornoFunzione
	End Function

	Private Function InserisceNuovoCampoInStringa(Stringa As String, Separatore As String, Posizione As Integer, Valore As String) As String
		Dim Ritorno As String = ""
		Dim Campi() As String = Stringa.Split(Separatore)
		Dim iPosizione As Integer = 0
		Dim Aggiunto As Boolean = False

		For Each s As String In Campi
			iPosizione += 1
			If iPosizione = Posizione Then
				Ritorno &= Valore & Separatore & s & Separatore
				Aggiunto = True
			Else
				Ritorno &= s & Separatore
			End If
		Next
		If Aggiunto Then
			Ritorno = Mid(Ritorno, 1, Ritorno.Length - 1)
		Else
			Ritorno = "ERROR: posizione non valida"
		End If
